...
const kfloat ACTIVITY_RANGE=PI/3.;
...
typedef struct {
	...
	kfloat b;
	kfloat c;
	kfloat (*force_t)(const kfloat,const kfloat,const kfloat,const kfloat);
} interaction_type;
...
namespace MPI {
	...
	void wait_any(int count,MPI_Request *req,int *index,MPI_Status *stat=MPI_STATUS_IGNORE) {
		twait.start();
	...
	template <class T>
	void isend(T *buff,int dest,int buffsize,int tag,MPI_Request *req) {
		MPI_Isend((char*)buff,buffsize,MPI_CHAR,dest,tag,MPI_COMM_WORLD,req);
	}
	
	template <class T>
	void irecv(T *buff,int source,int buffsize,int tag,MPI_Request *req) {
		MPI_Irecv((char*)buff,buffsize,MPI_CHAR,source,tag,MPI_COMM_WORLD,req);
	}
	...
	void allred(void *sbuff,void *rbuff,int count,MPI_Datatype d,MPI_Op op) {
		MPI_Allreduce(sbuff,rbuff,count,d,op,MPI_COMM_WORLD);
	}
	
	void reduce(void *sbuff,void *rbuff,int count,int root,MPI_Datatype dt,MPI_Op op) {
		MPI_Reduce(sbuff,rbuff,count,dt,op,root,MPI_COMM_WORLD);
	}
	
	void clock_assembly() {
		unsigned long long rwait,rstep,rinteract,rsearch,rforce,rposition;
		...
		reduce(&tinteract.sum,&rinteract,1,0,MPI_UNSIGNED_LONG_LONG,MPI_SUM);
		reduce(&tsearch.sum,&rsearch,1,0,MPI_UNSIGNED_LONG_LONG,MPI_SUM);
		reduce(&tforce.sum,&rforce,1,0,MPI_UNSIGNED_LONG_LONG,MPI_SUM);
		reduce(&tposition.sum,&rposition,1,0,MPI_UNSIGNED_LONG_LONG,MPI_SUM);
...
namespace vmath {
	...
	void pbc(kfloat *v) {
		for (int i=0;i<D;++i) v[i]-=std::floor(v[i]/parameters::box_x[i])*parameters::box_x[i];
	}
	
	kfloat distance(kfloat *v,kfloat *u,int i) {
		return v[i]-u[i]-std::floor((v[i]-u[i])/parameters::box_x[i]+0.5)*parameters::box_x[i];
	}
	
	kfloat abs_distance(kfloat *v,kfloat *u,int i) {
		return std::abs(v[i]-u[i]-std::floor((v[i]-u[i])/parameters::box_x[i]+0.5)*parameters::box_x[i]);
	}
	
	kfloat distance_squared(kfloat *v,kfloat *u) {
		kfloat d[D];
		for (int i=0;i<D;++i) d[i]=v[i]-u[i]-std::floor((v[i]-u[i])/parameters::box_x[i]+0.5)*parameters::box_x[i];
	...
	void set_sub_pbc(kfloat *v,kfloat *a,kfloat *b) {
		for (int i=0;i<D;++i) v[i]=a[i]-b[i]-std::floor((a[i]-b[i])/parameters::box_x[i]+0.5)*parameters::box_x[i];
	}
	...
	void rotate(kfloat *v,kfloat *a) {
		...
		v[1]=(c[0]*s[2]+c[2]*s[0]*s[1])*v_[0]+(c[0]*c[2]-s[0]*s[1]*s[2])*v_[1]-c[1]*s[0]*v_[2];
		v[2]=(s[0]*s[2]-c[0]*c[2]*s[1])*v_[0]+(c[2]*s[0]+c[0]*s[1]*s[2])*v_[1]+c[0]*c[1]*v_[2];
	}
	...
	void randomize_matrix(kfloat v[][D],rngSource &gen) {
		for (int i=0;i<D;++i) for (int j=0;j<D;++j) v[i][j]=gen.rNormFloat64();
	}
...
/*
		particle_type *ptype=&parameters::ptype_map[type];
		particle_type *qtype=&parameters::ptype_map[o.type];
		if (ptype->activity) {
			kfloat angle=std::acos(a,unit);
			if (angle<ACTIVITY_RANGE) {
				vmath::add_mul(force_ij,a,ptype->activity);
			}
		}
		if (qtype->activity) {
			kfloat angle=PI-std::acos(o.a,unit);
			if (angle<ACTIVITY_RANGE) {
				vmath::add_mul(force_ij,o.a,-qtype->activity);
			}
		}
*/
